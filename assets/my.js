...
$(document).ready(function () {
    ...
    $('#whatsappBtn').click(function (e) {
        ...
        if (!name) {
            showLabelError(nameField, 'Name is required');
            return;
        }

        if (!email) {
            showLabelError(emailField, 'Email is required');
            return;
        }

        if (!/^[^\s@]+@[^\s@]+\.[^\s@]{2,}$/.test(email)) {
            showLabelError(emailField, 'Enter a valid email address');
            return;
        }

        if (!phone) {
            showLabelError(phoneField, 'Phone is required');
            return;
        }

        if (!message) {
            showLabelError(messageField, 'Message is required');
            return;
        }

        // WhatsApp message
        var whatsappMessage = `Hello Vijendra Yadav %0A%0A`;
        whatsappMessage += `I would like to consult about a legal matter.%0A%0A`;
        whatsappMessage += `*Name:* ${name}%0A`;
        whatsappMessage += `*Phone:* ${phone}%0A`;
        whatsappMessage += `*Message:* ${message}%0A%0A`;
        whatsappMessage += `Please contact me at your earliest convenience.%0A%0A`;
        whatsappMessage += `Thank you!`;

        var whatsappNumber = '919829298773';
        window.open(`https://wa.me/${whatsappNumber}?text=${whatsappMessage}`, '_blank');
    });

    // Helper function
    function showLabelError(field, message) {
        field.addClass('error-border').focus();

        field.closest('.form-group')
            .find('label')
            .append(`<span class="label-error">${message}</span>`);
    }
